          ...
          {services.map((service, index) => (
            <Card key={index} className="bg-white hover:shadow-lg transition-shadow">
              <CardContent className="pt-6">
                ...
                <p className="text-gray-600 text-center">{service.description[language] || service.description.ru}</p>
              </CardContent>
            </Card>
          ))}
        </div>
        ...
        <div className="mt-16 bg-gov-blue rounded-lg overflow-hidden shadow-lg">
          <div className="p-8 text-center">
            <h3 className="text-2xl font-medium text-white mb-4">
              {language === 'ru' ? 'Видео о нашем центре' : 'Біздің орталық туралы бейне'}
            </h3>
            <div className="aspect-w-16 aspect-h-9 rounded-lg overflow-hidden">
              <div className="w-full h-0 pt-[56.25%] bg-gray-800 relative flex items-center justify-center">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-white text-opacity-70">
                    {language === 'ru' ? 'Здесь будет видео о центре' : 'Мұнда орталық туралы бейне болады'}
                  </div>
                </div>
              </div>
            </div>
